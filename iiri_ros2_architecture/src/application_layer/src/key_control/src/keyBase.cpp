 ...
 * @return {}
 */
void KeyBase::SaveGamepadBtn(const GamepadData &key)
{
    ...
    btn_["rt_lb"].SetValue((key.stickRt > 0.8) && key.btnLb);
    btn_["rt"].SetValue(key.stickRt > 0.8); // 将joystick抽象为按键
    btn_["lt"].SetValue(key.stickLt > 0.8);
...
std::map<std::string, double> KeyBase::GetStick()
{
    std::lock_guard<std::mutex> lock(mutex_);
    return stick_;
}