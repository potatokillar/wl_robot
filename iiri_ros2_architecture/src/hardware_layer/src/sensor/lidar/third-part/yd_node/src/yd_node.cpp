...
int main(int argc, char *argv[]) {
  ...
  auto node = rclcpp::Node::make_shared("yd_node");
  ...
  auto yd_node = node->create_publisher<sensor_msgs::msg::LaserScan>("/scan", rclcpp::SensorDataQoS());
  ...
  while (ret && rclcpp::ok()) {
    ...
    LaserScan scan;//
    RCLCPP_INFO(node->get_logger(), "SCAN_MSG->header.stamp.nanosec: ");
    if (laser.doProcessSimple(scan)) {
      auto scan_msg = std::make_shared<sensor_msgs::msg::LaserScan>();
      scan_msg->header.stamp.sec = RCL_NS_TO_S(scan.stamp);
      RCLCPP_INFO(node->get_logger(), "SCAN_MSG: %d", scan_msg->header.stamp.sec);
      scan_msg->header.stamp.nanosec =  scan.stamp - RCL_S_TO_NS(scan_msg->header.stamp.sec);
      RCLCPP_INFO(node->get_logger(), "SCAN_MSG->header.stamp.nanosec: %d", scan_msg->header.stamp.nanosec);
      RCLCPP_INFO(node->get_logger(), "SCAN_MSG->header.stamp.nanosec: ");
      std::cout << "scan_msg->header.stamp.nanosec" << std::endl;
