 ...
 * @return {}
 */
SerialPortInfo::SerialPortInfo(boost::asio::io_context& io) : fd(io)
{
    // txBuf.resize(256);
    rxBuf.resize(256);
}
...
bool SerialPort::Open(const std::string& name, const std::string& dev)
{
    ...
    try {
        ...
        auto logger = rclcpp::get_logger("SerialPort");
        RCLCPP_INFO(logger, "open error: name:%s, error: %s", dev.c_str(), e.what());
        return false;
    }
    return true;
}

bool SerialPort::SetParam(const std::string& name, uint32_t baudRate, int dataBits, Parity parity, StopBits nStop)
{
    try {
 ...
 * @description: 异步写
 * @param name
 * @param data 数据，请务必保证该内存的生命周期，在异步真正调用的时候是可用的
 * @return {}
 */
void SerialPort::AsyncWrite(const std::string& name, const std::vector<uint8_t>& data)
{
    try {
        auto& port = ports_.at(name);
...
/**
 * @description: 同步写--可以外接串口助手读取到树莓派"/dev/ttyAMA1"串口发送的数据
 * @param name
 * @return {}
 */
void SerialPort::Write(const std::string& name, const std::vector<uint8_t>& data)
{
    try {
...
/**
 * @description: 同步读--可以外接串口助手发送数据，树莓派"/dev/ttyAMA1"串口接收到数据，调用该函数读取到数据
 * @return {}
 */
std::vector<uint8_t> SerialPort::Read(const std::string& name)
{
    try {
        auto& port = ports_.at(name);
 ...
 * @param name
 * @param rxCnt
 * @param txCnt 目前该值无用，只是保留接口
 * @return {}
 */
void SerialPort::SetBufferSize(const std::string& name, uint32_t rxCnt, uint32_t txCnt)
{
    (void)txCnt;
    try {
        auto& port = ports_.at(name);
...
/**
 * @description: 启动事件循环，asio若发现没有任何可回调的事件，run就会立刻结束，因此每次设置异步事件都建议start一下
 * @return {}
 */
