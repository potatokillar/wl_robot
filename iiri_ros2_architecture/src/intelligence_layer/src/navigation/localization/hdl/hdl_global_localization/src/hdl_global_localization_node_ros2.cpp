...
public:
  GlobalLocalizationNode(rclcpp::NodeOptions& options, std::string name = std::string("hdl_global_localization_node")) : rclcpp::Node(name, options) {
    ...
    this->declare_parameter<std::string>("global_localization_engine", "BBS");

    globalmap_downsample_resolution = this->get_parameter("globalmap_downsample_resolution").as_double();
    query_downsample_resolution = this->get_parameter("query_downsample_resolution").as_double();

    set_engine(this->get_parameter("global_localization_engine").as_string());
  ...
  bool set_engine(const std::string& engine_name) {
    ...
    if (engine_name == "BBS") {
      engine = std::make_unique<GlobalLocalizationBBS>();
    } else if (engine_name == "FPFH_RANSAC") {
      engine = std::make_unique<GlobalLocalizationEngineFPFH_RANSAC>();
    } else {
  ...
  void set_global_map(const srv::SetGlobalMap::Request::SharedPtr req, srv::SetGlobalMap::Response::SharedPtr res) {
    if(global_map != nullptr) {
      RCLCPP_WARN(this->get_logger(), "Have set the global map");
      res->success = true;
      return;
    }
