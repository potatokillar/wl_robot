...
public:
  ...
  : Node("hdl_localization", options)
  {
    ...
    predict_cb_group = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
    sub_options.callback_group = predict_cb_group;
    if (use_imu) {
      RCLCPP_INFO(get_logger(), "enable imu-based prediction");
      imu_sub = create_subscription<sensor_msgs::msg::Imu>("/gpsimu_driver/imu_data", 256, std::bind(&HdlLocalizationNodelet::imu_callback, this, std::placeholders::_1), sub_options);
    }
    points_sub = create_subscription<sensor_msgs::msg::PointCloud2>("/velodyne_points", 5, std::bind(&HdlLocalizationNodelet::points_callback, this, std::placeholders::_1), sub_options);

    auto latch_qos = rclcpp::QoS(1).transient_local();
    globalmap_serv = create_service<hdl_localization::srv::SetGlobalMap>("/get_globalmap", std::bind(&HdlLocalizationNodelet::globalmap_callback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    if (use_global_localization) {
      global_localization_cb_group = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
      set_global_map_service = create_client<hdl_global_localization::srv::SetGlobalMap>("set_global_map", rmw_qos_profile_services_default, global_localization_cb_group);
      query_global_localization_service = create_client<hdl_global_localization::srv::QueryGlobalLocalization>("query", rmw_qos_profile_services_default, global_localization_cb_group);
      set_global_map_timer = create_wall_timer(std::chrono::milliseconds(200), std::bind(&HdlLocalizationNodelet::set_global_map_timer_callback, this));
...
private:
  ...
  void points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    ...
    const auto& stamp = points_msg->header.stamp; // frame_id 为数据源传感器坐标系，ros_bag数据即为base_link
   ...
   * @param global_map_msg recieved from GlobalmapServerNodelet
   * @param response response to recieved globalmap success or not 
   */
  void globalmap_callback(const std::shared_ptr<hdl_localization::srv::SetGlobalMap::Request> global_map_msg, std::shared_ptr<hdl_localization::srv::SetGlobalMap::Response> response) {
    if (globalmap) {
      response->success = true;
      RCLCPP_WARN(this->get_logger(), "Have recieved the global map !");
      return;
    }
    pcl::PointCloud<PointT>::Ptr cloud(new pcl::PointCloud<PointT>());
    // pcl::PointCloud<PointT>::Ptr cloud = boost::make_shared<pcl::PointCloud<PointT>>();
    pcl::fromROSMsg(global_map_msg->point_cloud_map, *cloud);

    if (cloud->points.empty()) {
      RCLCPP_WARN(this->get_logger(), "Recieved global map is empty!");
  ...
  void set_global_map_timer_callback() {
    ...
    if (!set_global_map_service->wait_for_service(std::chrono::milliseconds(5000))) {
      RCLCPP_WARN(get_logger(), "Waiting for SetGlobalMap service");
      if (!rclcpp::ok()) {
        RCLCPP_ERROR(get_logger(), "Node has been shutdown !");
        return;
      }
    }
    auto req = std::make_shared<hdl_global_localization::srv::SetGlobalMap::Request>();
    pcl::toROSMsg(*globalmap, req->global_map);
    auto result_future = set_global_map_service->async_send_request(req);

    auto result = result_future.get();
    if (result->success) {
      set_global_map_timer->cancel();
      RCLCPP_INFO(this->get_logger(), "Set global map to hdl_global_localization node success !");
    } else {
      RCLCPP_WARN(this->get_logger(), "Set global map to hdl_global_localization node failed !");
    }
  }
  ...
  bool relocalize(std::shared_ptr<std_srvs::srv::Empty::Request> req, std::shared_ptr<std_srvs::srv::Empty::Response> res) {
    ...
    if (!query_global_localization_service->wait_for_service(std::chrono::milliseconds(20))) {
      RCLCPP_ERROR(get_logger(), "QueryGlobalLocalization service not online !");
      return false;
    }
    ...
    // query_global_localization_service->async_send_request(query_req, std::bind(&HdlLocalizationNodelet::query_global_localization_call_back, this, std::placeholders::_1));
    auto query_future = query_global_localization_service->async_send_request(query_req);
  ...
  void query_global_localization_call_back(rclcpp::Client<hdl_global_localization::srv::QueryGlobalLocalization>::SharedFuture query_result_future) {
...
private:
  ...
  rclcpp::Service<hdl_localization::srv::SetGlobalMap>::SharedPtr globalmap_serv;
  ...
  rclcpp::CallbackGroup::SharedPtr global_localization_cb_group;
  rclcpp::SubscriptionOptions sub_options;
  rclcpp::CallbackGroup::SharedPtr predict_cb_group;
  ...
  rclcpp::TimerBase::SharedPtr set_global_map_timer;
