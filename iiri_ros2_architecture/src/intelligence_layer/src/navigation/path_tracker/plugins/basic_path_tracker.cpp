#include "path_tracker/plugins/basic_path_tracker.hpp"

namespace path_tracker {

void BasicPathTracker::configure(const std::shared_ptr<rclcpp_lifecycle::LifecycleNode> parent) {
    node_ = parent;
    flag_collision_occur_ = false;

    RCLCPP_INFO(node_->get_logger(), "BasicPathTracker configuration...");

    node_->declare_parameter<double>("path_tracker/BasicPathTracker.track_forward_distance", 0.5);
    node_->declare_parameter<double>("path_tracker/BasicPathTracker.fixed_velocity", 0.7);
    node_->declare_parameter<double>("path_tracker/BasicPathTracker.fixed_max_velocity", 1.0);
    node_->declare_parameter<double>("path_tracker/BasicPathTracker.yaw_vel_fixed", 1.57);
    node_->declare_parameter<double>("path_tracker/BasicPathTracker.tolerance_distance", 0.6);
    node_->declare_parameter<double>("path_tracker/BasicPathTracker.tolerance_back_distance", 0.4);
    node_->declare_parameter<int>("path_tracker/BasicPathTracker.time_init_ms", 400);
    node_->declare_parameter<int>("path_tracker/BasicPathTracker.time_brake_ms", 200);
    node_->declare_parameter<double>("path_tracker/BasicPathTracker.brake_vel", -1.5);

    track_forward_distance_ = node_->get_parameter("path_tracker/BasicPathTracker.track_forward_distance").as_double();
    fixed_velocity_ = node_->get_parameter("path_tracker/BasicPathTracker.fixed_velocity").as_double();
    fixed_max_velocity_ = node_->get_parameter("path_tracker/BasicPathTracker.fixed_max_velocity").as_double();
    yaw_vel_fixed_ = node_->get_parameter("path_tracker/BasicPathTracker.yaw_vel_fixed").as_double();
    tolerance_distance_ = node_->get_parameter("path_tracker/BasicPathTracker.tolerance_distance").as_double();
    tolerance_back_distance_ = node_->get_parameter("path_tracker/BasicPathTracker.tolerance_back_distance").as_double();
    time_init_ms_ = node_->get_parameter("path_tracker/BasicPathTracker.time_init_ms").as_int();
    time_brake_ms_ = node_->get_parameter("path_tracker/BasicPathTracker.time_brake_ms").as_int();
    brake_vel_ = node_->get_parameter("path_tracker/BasicPathTracker.brake_vel").as_double();

    collision_detect_sub_ = node_->create_subscription<std_msgs::msg::Bool>("collision_occur", 10,
        std::bind(&BasicPathTracker::recv_closest_point, this, std::placeholders::_1)
    );
}

void BasicPathTracker::recv_closest_point(const std::shared_ptr<std_msgs::msg::Bool> collision_occur_msg) {
...
std::optional<geometry_msgs::msg::Twist> BasicPathTracker::compute_cmd_vel(const geometry_msgs::msg::Pose & current_pose, const geometry_msgs::msg::Twist & current_vel) {
    ...
    // Simple geometric calculation instead of Ceres optimization
    double target_angle = std::atan2(target_point_current(1), target_point_current(0));
    double angular_velocity = target_angle * 2.0; // Simple proportional control
    
    // Limit angular velocity
    if (angular_velocity > yaw_vel_fixed_) {
        angular_velocity = yaw_vel_fixed_;
    } else if (angular_velocity < -yaw_vel_fixed_) {
        angular_velocity = -yaw_vel_fixed_;
    }
...
// Copy other methods from SimplePathTracker
double BasicPathTracker::get_closest_distance(const int & closest_idx, const geometry_msgs::msg::Pose & current_pose) {
...
void BasicPathTracker::set_path(const nav_msgs::msg::Path & path) {
...
int BasicPathTracker::get_target_idx(const int & closest_idx, const double & track_forward_distance) {
...
double BasicPathTracker::get_fit_length(const Eigen::MatrixXd & path_segment) {
    double fit_length = 0;
...
std::optional<std::pair<int, int>> BasicPathTracker::get_segment_idx(const Eigen::Vector3d & current_position) {
...
int BasicPathTracker::get_closest_idx(const Eigen::Vector3d & current_position, std::pair<int, int> & segment_idx) {
...
BasicPathTracker::~BasicPathTracker() {
    RCLCPP_WARN(node_->get_logger(), "BasicPathTracker deconstructing...");
    node_.reset();
}
...
PLUGINLIB_EXPORT_CLASS(path_tracker::BasicPathTracker, navigation_core::BasePathTracker)