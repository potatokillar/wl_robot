...
def generate_launch_description():
    ...
    realworld_experiment_arg = DeclareLaunchArgument('realworld_experiment', default_value=realworld_experiment, description='real quadrotor experiment')
    ...
    ego_planner_node = Node(
        ...
        parameters=[
            {'fsm/flight_type': flight_type}, # manual_target 为手动设置目标点，preset_target 为根据预设的路点进行路径规划，refence_path 应该是根据参考路径飞行
            {'fsm/thresh_replan_time': 1.0}, # 重规划超时时间，如果小于改时间则不需要重规划，正常前进
            {'fsm/thresh_no_replan_meter': 1.0}, # 重规划距离，如果小于该距离则到达终点，或者向下一个目标点进行路径规划
            {'fsm/planning_horizon': planning_horizon}, # 规划局部路径的最大距离范围，每次规划局部路径时，将全局路径上超过该范围的第一个点作为local_target_pos;
            {'fsm/planning_horizen_time': 3.0}, # 没有用到
            {'fsm/emergency_time': 1.0}, # 当前时间与轨迹上发现障碍物之间的最小时间
            {'fsm/realworld_experiment': realworld_experiment}, # 是否真实样机实验，true: have_trigger_ = false,即需要触发
            {'fsm/fail_safe': True},
