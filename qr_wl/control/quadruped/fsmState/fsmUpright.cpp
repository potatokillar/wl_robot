...
FsmUpright::FsmUpright(ContrContainer contr, std::shared_ptr<JpData> jpData) : FsmState(AlgoState::upright, contr, jpData)
{
    ...
    if (enable) {
        ...
        onnxPath_ = GetRobotConfigDirect<std::string>("onnxPath", "upright").value();
        defaultJoint_ = GetRobotConfigDirect<vector<double>>("defaultJoints", "upright").value();
        kFilter_ = GetRobotConfigDirect<double>("kFilter", "upright").value();
        if (!isWheel_) {
            aiSend_.k2.fill(GetRobotConfigDirect<double>("kp", "upright").value());
            aiSend_.k1.fill(GetRobotConfigDirect<double>("kd", "upright").value());
        } else {
            std::vector<double> kp = GetRobotConfigDirect<std::vector<double>>("kp", "upright").value();
            std::vector<double> kd = GetRobotConfigDirect<std::vector<double>>("kd", "upright").value();
...
void FsmUpright::OnExit()
{
    LOG_INFO("upright exit");
    iRun_ = 0;
    interval_ = 0;
}
...
void FsmUpright::LeggedWheelRun()
{
    auto start = std::chrono::high_resolution_clock::now();
...
void FsmUpright::NetWorkResult(std::vector<double> imu, float vx, float vy, float wz, DataCmd motorRet)
{
...
void FsmUpright::NetWorkResult(std::vector<double> imu, float vx, float vy, float wz, CmdVal4 wheelMotorRet)
{
    std::vector<const char*> output_node_names = {"output"};
    std::vector<const char*> input_node_names = {"input"};
    std::vector<float> out_cmd_val(input_tensor_size_);
...
DataCmd FsmUpright::SetLeggedPID()
{
    // 生成下发指令的数据结构
    DataCmd motorCmd;
...
CmdVal4 FsmUpright::SetLeggedWheelPID()
{
...
std::vector<double> FsmUpright::transXYZ(double x, double y, double z, double w)
{
...
void FsmUpright::PrintNetworkResult(std::vector<float> out_cmd_val)
{
    if (!isWheel_) {
