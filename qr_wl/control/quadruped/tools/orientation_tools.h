 ...
 * 为坐标系转换计算旋转矩阵，注意：该函数输入0.1rad得到的矩阵会把矢量v旋转-0.1rad，这个转换相当于坐标系旋转了0.1rad
 */
template <typename T>
...
/*!
 * Go from rpy to rotation matrix.,Rx/Ry/Rz与百度百科表达式相差转置!!
 * 注意,RxT*RyT*RzT = (Rz*Ry*Rx)T,因此该结果是世界转身体，身体转世界，请转置
 * 解释1：坐标系不动，对坐标系下的矢量进行-RPY的旋转操作，表达在(未旋转)的身体坐标系下。
 * 解释2：矢量不动，对身体坐标系进行RPY的旋转操作，表达在(旋转后的)目标坐标系下。
 * 采用哪种解释取决于结果矢量的坐标系。
 * 这里采用Rx*Ry*Rz(r,p,y)，即XYZ欧拉角来表述是为了方便使用，不同于书本的ZYX欧拉角表述，都可以表述姿态。
 * 需要注意，区别X-X-X固定角(滚转、俯仰、偏航指的是固定)表示法    X-X-X欧拉角表示法
 *
 * MIT...RxT*RyT*RzT
 * Matlab...Rz*Ry*Rx(默认ZYX输入变量！！），两者相差转置
 */
template <typename T>
...
/*!
 * Convert a 3x1 vector to a skew-symmetric 3x3 matrix   旋转向量->反对称矩阵(R3 -> so(3))
 */
template <typename T>
...
/*!
 * Put the skew-symmetric component of 3x3 matrix m into a 3x1 vector  反对称矩阵->旋转向量  (so(3)->R3)
 */
template <typename T>
...
/*!
 * Convert a coordinate transformation matrix to an orientation quaternion.    旋转矩阵转四元数
 */
template <typename T>
 ...
 * angles in (roll, pitch, yaw).  四元数转欧拉角(r,p,y)
 */
template <typename T>
...
/*!
 * Quaternion derivative calculation, like rqd(q, omega) in MATLAB.四元数的导数
...
/*!
 * Take the product of two quaternions 四元数相乘
 */
template <typename T>
